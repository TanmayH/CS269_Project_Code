...
class RandomHtml: #TODO: Remove wg-detect classes, not in use anymore
	def mixer(n_elements, section_iteration,section_component_detail, with_annotations=False): 
		...
		if(n_elements==1):
			section_component_detail["mixed-single"] = element_layout_list
		else:
			section_component_detail["mixed-double"] = element_layout_list
	...
	def element_creator(ele,section_iteration,element_layout_list, with_annotations=False):
		...
		if ele == wcc.Mixable.Heading: #TODO: Populate with RandomContent
			...
			element_layout_list.append({"mix-heading":["label"]})
			...
			element_layout_list.append({"mix-para":["label"]})
		# elif ele == "button":
		...
		# 	element_layout_list.append({"mix-feature":["feature-element"]})
			...
			count_list = RandomContent.links(count_range=(3,4),length_range=(13,13), range_in_chars=True)
			...
			titles  = RandomContent.titles_descriptions((2,3))
	...
	def cards(n_items, section_component_detail, with_annotations=False):
		...
		with_placeholder = False #temp removing with placeholder versions
		shadow_class = " shadow" if rh.flip_coin() else ""
			...
			if n_cols > 1:
				...
				for j in range(n_cols):
					...
					if form_input["type"] == wcc.InputTypes.Choicer:
						...
						# col_component_values.insert(0, "label")
				...
				content_components.append({"row-double": row_components})
				...
				if form_input["type"] == wcc.InputTypes.Choicer:
					...
					col_component_values = ["choice-option-input" for i in range(len(form_input["choices"]))]
					# col_component_values.insert(0, "label")
				...
				row_components.append({col_component :  col_component_values})
				content_components.append({"row-single":row_components})	
				# content_components.append({"row": row_components})
	...
	def sections(n_sections, content_components, with_annotations=False):
		...
		for i in range(n_sections):
			...
			if section_elements == wcc.Composed.Carousel and (section_elements not in created_elements):
				...
				tab_dict = RandomContent.random_dictionary((2,3))
				...
				RandomHtml.cards(random.randint(2,3), section_component_detail, with_annotations=with_annotations)
