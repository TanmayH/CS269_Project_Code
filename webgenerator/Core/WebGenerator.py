...
class WebGenerator:
	...
	def generate(self, with_annotations=None, with_color_variation=None, output_path=None):
		...
		if "sidebar" in random_website.force_sizes: #TODO: Move options and probabilities 
			...
			links = RandomContent.links(count_range=(2,3),length_range=(13,13), range_in_chars=True) #2,6
		...
		if "navbar" in random_website.force_sizes:
			...
			links = RandomContent.links(count_range=(2,3),length_range=(13,13), range_in_chars=True) #2,3
			with_heading =  rh.flip_coin()
	...
	def write_to_gui_file(self, layout_list):
		for component in layout_list:
			print(component)
		print ("\n\n")
		output_path = os.path.join(self.output_path, "gui")
		gui_string = ""
		for component in layout_list:
		...
		with open(os.path.join(output_path, "random_webpage.gui"), 'a') as convert_file:
